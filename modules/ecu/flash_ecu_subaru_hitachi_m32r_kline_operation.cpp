#include "flash_ecu_subaru_hitachi_m32r_kline_operation.h"
...
bool FlashEcuSubaruHitachiM32rKlineOperation::execute()
{
 ...
 * @return success
 */
int FlashEcuSubaruHitachiM32rKlineOperation::connect_bootloader_subaru_ecu_hitachi_kline()
{
    QByteArray received;
    QByteArray seed;
    QByteArray seed_key;
 ...
 *
 */
int FlashEcuSubaruHitachiM32rKlineOperation::read_mem(uint32_t start_addr, uint32_t length)
{
    QElapsedTimer timer;
...
int FlashEcuSubaruHitachiM32rKlineOperation::write_mem(bool test_write)
{
    QByteArray received;
    QByteArray filedata;
...
int FlashEcuSubaruHitachiM32rKlineOperation::reflash_block(const uint8_t *newdata, const struct flashdev_t *fdt, unsigned blockno, bool test_write)
{
    ...
    for (int i = 0; i < 20; i++)
    {
        received.append(serial->read_serial_data(receive_timeout));
        emit LOG_I(".", false, false);
        if (received.length() > 5)
        {
...
//    return STATUS_SUCCESS;
    ...
    serial->write_serial_data_echo_check(output);
    delay(50);
    received = serial->read_serial_data(1000);
    if (received != "")
    {
 ...
 * @return received response
 */
QByteArray FlashEcuSubaruHitachiM32rKlineOperation::send_subaru_sid_a0_block_read(uint32_t dataaddr, uint32_t datalen)
{
    QByteArray output;
    QByteArray received;
    QByteArray msg;
 ...
 * @return seed key (4 bytes)
 */
QByteArray FlashEcuSubaruHitachiM32rKlineOperation::calculate_seed_key(QByteArray requested_seed, const uint16_t *keytogenerateindex, const uint8_t *indextransformation)
{
    QByteArray key;
 ...
 */
QByteArray FlashEcuSubaruHitachiM32rKlineOperation::send_sid_bf_ssm_init()
{
    QByteArray output;
    QByteArray received;
 ...
 * @return received response
 */
QByteArray FlashEcuSubaruHitachiM32rKlineOperation::send_subaru_sid_81_start_communication()
{
    QByteArray output;
    QByteArray received;
 ...
 * @return received response
 */
QByteArray FlashEcuSubaruHitachiM32rKlineOperation::send_subaru_sid_83_request_timings()
{
    QByteArray output;
    QByteArray received;
 ...
 * @return seed (4 bytes)
 */
QByteArray FlashEcuSubaruHitachiM32rKlineOperation::send_subaru_sid_27_request_seed()
{
    QByteArray output;
    QByteArray received;
 ...
 * @return received response
 */
QByteArray FlashEcuSubaruHitachiM32rKlineOperation::send_subaru_sid_10_start_diagnostic()
{
    QByteArray output;
    QByteArray received;
...
QByteArray FlashEcuSubaruHitachiM32rKlineOperation::send_subaru_sid_b8_change_baudrate_38400()
{
    QByteArray output;
    QByteArray received;
    QByteArray msg;
 ...
 */
QByteArray FlashEcuSubaruHitachiM32rKlineOperation::add_ssm_header(QByteArray output, uint8_t tester_id, uint8_t target_id, bool dec_0x100)
{
    QByteArray received;
    QByteArray msg;
 ...
 */
uint8_t FlashEcuSubaruHitachiM32rKlineOperation::calculate_checksum(QByteArray output, bool dec_0x100)
{
    uint8_t checksum = 0;
 ...
 * @return parsed message
 */
QString FlashEcuSubaruHitachiM32rKlineOperation::parse_message_to_hex(QByteArray received)
{
    QString msg;
