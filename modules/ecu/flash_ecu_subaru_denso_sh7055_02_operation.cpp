...
bool FlashEcuSubaruDensoSH7055_02Operation::execute()
{
...
int FlashEcuSubaruDensoSH7055_02Operation::connect_bootloader()
{
    QByteArray output;
    QByteArray received;
    ...
    serial->set_lec_lines(serial->get_requestToSendDisabled(), serial->get_dataTerminalDisabled());
    serial->read_serial_data(10);
    confirm(tr("ECU Operation"), "Connecting to bootloader, turn ign off / on and click OK to continue", QMessageBox::Ok, QMessageBox::Ok);
...
int FlashEcuSubaruDensoSH7055_02Operation::upload_kernel(QString kernel, uint32_t kernel_start_addr)
{
    QFile file(kernel);
 ...
 * @return success
 */
int FlashEcuSubaruDensoSH7055_02Operation::read_mem(uint32_t start_addr, uint32_t length)
{
    QElapsedTimer timer;
    QByteArray output;
    QByteArray received;
 ...
 * @return success
 */
int FlashEcuSubaruDensoSH7055_02Operation::write_mem(bool test_write)
{
    QByteArray filedata;
    QByteArray output;
    QByteArray received;
 ...
 * @return
 */
int FlashEcuSubaruDensoSH7055_02Operation::get_changed_blocks(const uint8_t *src, int *modified)
{
    unsigned blockno;
    QByteArray msg;
    for (blockno = 0; blockno < flashdevices[mcu_type_index].numblocks; blockno++) {
 ...
 * @return
 */
int FlashEcuSubaruDensoSH7055_02Operation::check_romcrc(const uint8_t *src, uint32_t start, uint32_t len, int *modified)
{
    QByteArray output;
    QByteArray received;
    QByteArray msg;
...
unsigned int FlashEcuSubaruDensoSH7055_02Operation::crc32(const unsigned char *buf, unsigned int len)
{
...
void FlashEcuSubaruDensoSH7055_02Operation::init_crc32_tab(void)
{
    uint32_t i, j;
    uint32_t crc, c;
...
int FlashEcuSubaruDensoSH7055_02Operation::init_flash_write()
{
    QByteArray output;
    QByteArray received;
    QByteArray msg;
 ...
 *  @return
 */
int FlashEcuSubaruDensoSH7055_02Operation::reflash_block(const uint8_t *newdata, const struct flashdev_t *fdt, unsigned blockno, bool test_write)
{
    uint32_t start = 0;
    uint32_t len = 0;
    uint32_t datalen = 0;
 ...
 * @return
 */
int FlashEcuSubaruDensoSH7055_02Operation::flash_block(const uint8_t *src, uint32_t start, uint32_t len)
{
    uint32_t remain = len;
 ...
 * @return kernel id
 */
QByteArray FlashEcuSubaruDensoSH7055_02Operation::request_kernel_id()
{
    QByteArray output;
    QByteArray received;
    QByteArray msg;
 ...
 */
QByteArray FlashEcuSubaruDensoSH7055_02Operation::send_sid_bf_ssm_init()
{
    QByteArray output;
    QByteArray received;
 ...
 * @return seed (4 bytes)
 */
QByteArray FlashEcuSubaruDensoSH7055_02Operation::send_sid_27_request_seed()
{
    QByteArray output;
    QByteArray received;
    QByteArray msg;
 ...
 */
QByteArray FlashEcuSubaruDensoSH7055_02Operation::send_sid_27_send_seed_key(QByteArray seed_key)
{
    QByteArray output;
    QByteArray received;
    QByteArray msg;
 ...
 */
QByteArray FlashEcuSubaruDensoSH7055_02Operation::add_ssm_header(QByteArray output, uint8_t tester_id, uint8_t target_id, bool dec_0x100)
{
 ...
 */
uint8_t FlashEcuSubaruDensoSH7055_02Operation::calculate_checksum(QByteArray output, bool dec_0x100)
{
    uint8_t checksum = 0;
...
int FlashEcuSubaruDensoSH7055_02Operation::check_received_message(uint8_t *req_response, QByteArray received)
{
    if (received.length() != 3)
    {
 ...
 * @return
 */
int FlashEcuSubaruDensoSH7055_02Operation::connect_bootloader_start_countdown(int timeout)
{
 ...
 * @return parsed message
 */
QString FlashEcuSubaruDensoSH7055_02Operation::parse_message_to_hex(QByteArray received)
{
    QString msg;
