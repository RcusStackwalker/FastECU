...
- Preserve caller-owned transport lifetime from ADR 0015: `BoundFlashAttempt` calls initial `configure`, initial `open`, and final `close`. Petrol and DensoCAN require a protocol-specific reset before that initial configuration; petrol adds no delay, while diesel retains its separate reset → 500 ms → configure/open sequence unchanged. DensoCAN's two mode-transition methods and the TCU's post-upload ISO restart encapsulate their protocol-required inner reset/configure/reopen operations; neither changes outer ownership.
...
- TCU order after positive kernel start is reset → configure ISO-15765 with `request_id=0x7E1`, `response_id=0x7E9`, `bitrate=500000`, and `extended_id=false` → open → cancellable 500 ms wait → strict kernel-ID verification. Reset/configure/open failures are terminal, and the bound attempt still performs the one final close.
