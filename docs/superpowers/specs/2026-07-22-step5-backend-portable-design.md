...
4 was a mechanical container-type substitution across nine packages. Step 5 is
a dependency inversion plus god-object decomposition spanning several
independent subsystems: a port/error foundation, ~30 flash operation families,
a 3,397-line `FileActions`, the logging path, and thread-ownership removal.
That does not fit one plan. This document fixes the shared vocabulary once —
