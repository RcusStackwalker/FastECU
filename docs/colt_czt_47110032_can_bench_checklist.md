...
3. **Write-path preflight on the bench ECU (not a vehicle).** Select Write
   with a known-good ROM file exactly `0x80000` (512 KiB) bytes. Confirm an
   image one byte shorter or longer is rejected before any confirmation or
   worker starts. Confirm the warning states that file bytes
   `0x0000`-`0x8000` (the first 32 KiB) are ignored, the ECU bootloader stays
   unchanged, and only `0x8000`-`0x80000` is written and verified. Then confirm
...
4. **Top 128KB bootstrap — second highest-risk step, new in this
   ...
   needed. The conditional-bootstrap warning is collected before execution;
   if a mismatch is found, the redirect path sends the
...
5. **Erase trigger (main write) — the original highest-risk step, now
   ...
   warning dialog appears before the worker starts and therefore before the
   `ServiceRequestReflash`(0x3B) unlock
   ...
   continuing.
   - Confirm cancellation immediately after either erase is reported without
     another write attempt. The warning must make clear that cancellation
     after erase can leave an incomplete image requiring recovery.
   - Confirm write progress is monotonic on one `0/0x78000` through
     `0x78000/0x78000` scale. Internal comparison and verification reads and
     RAM-helper uploads must not reset or advance that scale.
6. **Userspace write + automatic read-back.** Once Step 5's erase trigger
   succeeds, confirm the userspace write (`0x8000`-`0x60000`) completes and
   the executor's immediate byte-for-byte verification reports "Userspace
   flash verified". The top 128KB verification is complete from Step 4 (the
   "Top 128KB verified" log line confirms the bootstrap matched on
   re-read inside `ensureTopRegionWritten()`).
   Confirm no read, erase, or write request targets below `0x8000` throughout
   the complete run.
