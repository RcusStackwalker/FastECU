...
inside a single bootloader session (one `0x10`/`0x27` handshake); an explicit
`connect` is accepted only as the first session step and cannot be repeated.
Steps run as separate invocations each reconnect from scratch, which is a
materially different sequence on the wire. Do not treat "ran individually"
as equivalent to "ran chained." In particular, a fresh process must never
infer that ECU RAM still contains the erase routine uploaded by an earlier
process.
...
- [x] `write-redirect`: 176-byte, checksum-`0x4eae` routine verified by writing
  ...
  **Retained evidence.** The readback files under
  `/private/tmp/fastecu-bench-verify` reproduce the recorded top-region
  SHA-256 `09d69fe0357997a97a843d1d8e16926d23d5941223a92fd6172d698119711380`
  and userspace SHA-256
  `0e572f84796cc55c40f23ffd7506957c5f9a7ed75dc5f0e9e4a5662464c141c0`. **No raw
  CAN trace of that session was retained.** The evidence therefore covers the
  redirect helpers' effect on flash contents and nothing about the frame-level
  exchange; the `MUT_DMA`/Colt hardware paths keep their **experimental**
  status, and neither the CLI nor the desktop workflow is qualified end to end
  by this run.
...
invocation, treat RAM contents as unknown. The CLI enforces this exact sequence
for a built-in erase helper before opening a session and also tracks successful
upload and unlock outcomes at runtime. A custom `--from` helper, a failed step,
or any intervening destructive command invalidates the sequence, so
`--keep-going` cannot reach erase with stale or unknown RAM.
...
- [ ] `send-raw` bypasses NRC handling entirely. A negative response (e.g. a
      ...
      code for this one command. Because arbitrary PDUs cannot be proven
      read-only, both `send` and `send-raw` require their own per-step
      `--destructive` acknowledgement. They still reject known destructive
      PDUs at parse time: SID `0x3B`, SID `0x34`, SID `0x36`, and RoutineControl
      `0x31/0xE0` are reserved for the named, gated commands.
