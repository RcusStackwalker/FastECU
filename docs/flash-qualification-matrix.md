...
| FlashEcuMitsuM32rCan | ECU | ISO-15765 | read, write | no | — | unqualified | — | Class `FlashEcuMitsuM32rCanOperation` (`src/platform/desktop/common/flash/legacy/ecu/flash_ecu_mitsu_m32r_can_operation.{h,cpp}`) implements both `mitsu_ecu_m32r_can` and `mitsu_ecu_m32r_can_vendor_ext` — `mainwindow.cpp` selects the vendor security-access challenge via a bool flag, not a different class. protocols.cfg declares `test_write=no` for both. A bench-qualification checklist exists at [`docs/colt_czt_47110032_can_bench_checklist.md`](colt_czt_47110032_can_bench_checklist.md) describing it as the worker-thread migration's pilot module, but the doc gates future real-vehicle use — it is not itself a completed-testing report, so `hardware_status` stays `unqualified`. |
