// Dialog orchestration tests for EepromEcuSubaruDensoSH705xCan (step 5c,
// Task 19b -- coverage follow-through). CAN sibling of
// test_eeprom_ecu_subaru_denso_sh705x_kline_dialog.cpp (this directory);
// same rationale and harness -- exercises the dialog's one-attempt-per-mode
...
// constructor signature is unchanged, so these seams are exposed via a test
// subclass overriding virtuals rather than an extra constructor parameter.
...
#include "test_eeprom_ecu_subaru_denso_sh705x_can_dialog.h"

using fastecu::ErrorKind;
using fastecu::FakeClock;
using fastecu::flash::DensoSecurityVariant;
using fastecu::flash::DensoSh705xEepromCanPlan;
...
class ScriptedExecutor : public fastecu::flash::IFlashExecutor
{
  public:
                                                  ...
                                                  fastecu::IEventSink&) override
    {
        if (seenModes)
        {
            seenModes->push_back(std::get<DensoSh705xEepromCanPlan>(plan.family_plan()).mode);
        }
        return nextResult;
    }
};

// Every field here matches an SH7055 CAN/ISO-15765 EEPROM read plan that
...
// satisfy. Unlike the K-Line plan, the CAN branch of
// build_denso_sh705x_eeprom_plan() has no security-variant restriction
// (kline_supports() only gates the K-Line branch), so Stock is fine here too.
fastecu::Result<FlashPlan> makePlan(EepromReadMode mode)
{
    return fastecu::flash::build_denso_sh705x_eeprom_plan(DensoSh705xEepromInput{
        ...
        .target_id = "sub_ecu_eeprom_denso_sh7055_can",
        .mcu_name = "SH7055",
        .flash_method = "sub_ecu_eeprom_denso_sh7055_can",
...
class TestableCanDialog : public EepromEcuSubaruDensoSH705xCan
{
  public:
    using EepromEcuSubaruDensoSH705xCan::EepromEcuSubaruDensoSH705xCan;
...
class TestEepromCanDialog : public QObject
{
    Q_OBJECT
  ...
  private slots:
    ...
    void modeOrchestrationIsExactlyTwoThreeFourNeverRepeatsOrSkips()
    {
        ...
        TestableCanDialog dialog(nullptr, &ecuCalDef, "read");
    ...
    // Regression coverage (mirrors the K-Line sibling's identically-named
    // test): this dialog's buildPlan() unconditionally calls
    // adapter.build_read_plan() -- it has no write path at all. MainWindow
    // dispatches this dialog by protocol name only, and cmd_type flows
    // through unfiltered from the generic top-level menu commands, so a user
    // who selects "Write ROM to ECU" (cmd_type "write") or "Test write ROM
    // to ECU" (cmd_type "test_write") on this protocol must never reach a
    // real EEPROM read -- the dialog must reject before buildPlan()/
    // makeWorker() and must never touch FullRomData.
    ...
    void nonReadCmdTypeIsRejectedBeforeBuildingAnyPlanOrWorker()
    {
        ...
        FileActions::EcuCalDefStructure ecuCalDef;
        TestableCanDialog dialog(nullptr, &ecuCalDef, cmdType);
...
int run_test_eeprom_ecu_subaru_denso_sh705x_can_dialog(int argc, char **argv)
{
    ...
    TestEepromCanDialog t;
    return QTest::qExec(&t, argc, argv);
}
#include "test_eeprom_ecu_subaru_denso_sh705x_can_dialog.moc"
