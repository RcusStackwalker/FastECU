...
EcuOperations::EcuOperations(QWidget *ui, SerialPortActions *serial_arg, QString mcu_type_string,
                             int mcu_type_index_arg)
{
    this->setParent(ui);
    this->setAttribute(Qt::WA_QuitOnClose, true);
    this->mcu_type_string = std::move(mcu_type_string);
    this->mcu_type_index = mcu_type_index_arg;
 ...
 ******************************************************/
int EcuOperations::read_mem_16bit_kline(FileActions::EcuCalDefStructure *ecuCalDef, uint32_t start_addr,
                                        uint32_t length)
{
    QElapsedTimer timer;
    QByteArray output;
 ...
 ******************************************************/
int EcuOperations::read_mem_32bit_kline(FileActions::EcuCalDefStructure *ecuCalDef, uint32_t start_addr,
                                        uint32_t length)
{
    QElapsedTimer timer;
    QByteArray output;
...
int EcuOperations::write_mem_16bit_kline(FileActions::EcuCalDefStructure *ecuCalDef, bool test_write)
{
    ...
    if (get_changed_blocks_16bit_kline(&data_array[0], block_modified.data()))
    {
    ...
    if (bcnt)
    {
        ...
        if (!test_write)
        {
            if (bcnt)
            {
                ...
                emit LOG_I("Don't power off your ECU, kernel is still running and you can try flashing again!", true,
                           true);
            }
        }
        else
...
int EcuOperations::write_mem_32bit_kline(FileActions::EcuCalDefStructure *ecuCalDef, bool test_write)
{
    ...
    if (get_changed_blocks_32bit_kline(&data_array[0], block_modified.data()))
    {
        emit LOG_I("Error in ROM compare", true, true);
        return STATUS_ERROR;
    }
...
int EcuOperations::write_mem_32bit_can(FileActions::EcuCalDefStructure *ecuCalDef, bool test_write)
{
    ...
    if (get_changed_blocks_32bit_can(&data_array[0], block_modified.data()))
    {
        emit LOG_I("Error in ROM compare", true, true);
        return STATUS_ERROR;
    }
...
int EcuOperations::check_romcrc_16bit_kline(const uint8_t *src, uint32_t start, uint32_t len, int *modified)
{
    ...
    for (uint32_t i = 0; i < len; i += ROMCRC_ITERSIZE_16BIT, chunko += ROMCRC_NUMCHUNKS_16BIT)
    {
        ...
        for (chunk_cnt = 0; chunk_cnt < ROMCRC_NUMCHUNKS_16BIT; chunk_cnt++)
        {
            ...
            // qDebug() << hex << chunk_cnt << received.length() << parse_message_to_hex(received) << "->" << hex <<
            // imgcrc;
            qDebug() << Qt::hex << chunk_cnt << received.length() << parse_message_to_hex(received) << romcrc << "->"
                     << Qt::hex << imgcrc;
...
int EcuOperations::check_romcrc_32bit_can(const uint8_t *src, uint32_t start_addr, uint32_t len, int *modified)
{
    ...
    for (; len > 0; len -= ROMCRC_ITERSIZE_CAN, chunko += ROMCRC_NUMCHUNKS_CAN)
    {
        ...
        if ((uint8_t)received.at(0) != SUB_DENSOCAN_START_COMM ||
            ((uint8_t)received.at(1) & 0xF8) != SID_CAN_CONF_CKS || chk_sum == (uint8_t)received.at(2))
        {
            continue;
        }
...
int EcuOperations::npk_raw_flashblock_16bit_kline(const uint8_t *src, uint32_t start, uint32_t len)
{
    ...
    if ((len & (blocksize - 1)) || (start & (blocksize - 1)))
    {
    ...
    while (remain)
    {
        ...
        {
            // maybe negative response, if so, get the packet
...
int EcuOperations::npk_raw_flashblock_32bit_can(const uint8_t *src, uint32_t start, uint32_t len)
{
    ...
    for (int i = 0; i < num_128_byte_blocks; i++)
    {
        ...
        for (int j = 0; j < 16; j++)
        {
            ...
            byte_index += 8;
            received = serial->write_serial_data_echo_check(output);
            // qDebug() << "Flash data message sent to kernel for block (128 byte block, 8 byte block): " +
            // QString::number(i) + "/" + QString::number(num_128_byte_blocks) + ", " + QString::number(j); qDebug() <<
            // parse_message_to_hex(output); delay(5);
        }
 ...
 ******************************************************/
int EcuOperations::reflash_block_16bit_kline(const uint8_t *newdata, const struct flashdev_t *fdt, unsigned blockno,
                                             bool test_write_arg)
{
    int errval;
    ...
    // 3- erase block //
    msg = QString("Erasing block %1 (0x%2-0x%3)...")
              .arg((uint8_t)blockno, 2, 16, QLatin1Char('0'))
              .arg((uint8_t)(unsigned)start, 8, 16, QLatin1Char('0'))
              .arg((uint32_t)(unsigned)start + len - 1, 8, 16, QLatin1Char('0'))
              .toUtf8();
    output.clear();
    output.append(SID_FLASH);
    ...
    if (errval)
    {
        emit LOG_I("Reflash error! Do not panic, do not reset the ECU immediately. The kernel is most likely still "
 ...
 ******************************************************/
int EcuOperations::reflash_block_32bit_kline(const uint8_t *newdata, const struct flashdev_t *fdt, unsigned blockno,
                                             bool test_write_arg)
{
    int errval;
 ...
 ******************************************************/
int EcuOperations::reflash_block_32bit_can(const uint8_t *newdata, const struct flashdev_t *fdt, unsigned blockno,
                                           bool test_write_arg)
{
    int errval;
 ...
 *
 ******************************************************/
int EcuOperations::read_mem_uj20_30_40_70_kline(FileActions::EcuCalDefStructure *ecuCalDef, uint32_t start_addr,
                                                uint32_t length)
{
    QElapsedTimer timer;
    QByteArray output;
...
static std::array<uint16_t, 256> crc_tab16;
...
static constexpr auto crc_table = std::to_array<unsigned int>(
