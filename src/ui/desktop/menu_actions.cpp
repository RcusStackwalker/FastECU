...
int MainWindow::simulate_obd()
{
    ...
    static constexpr auto sid_4d_ff_b4 = std::to_array<uint8_t>({0x4c, 0x00, 0xb4});

    static constexpr auto sid_81 = std::to_array<uint8_t>({0x83, 0xf1, 0x12, 0xc1, 0xef, 0x8f});
    static constexpr auto sid_82 = std::to_array<uint8_t>({0x81, 0xf1, 0x12, 0xc2});
    static constexpr auto sid_83_00 =
        std::to_array<uint8_t>({0x80, 0xf1, 0x12, 0x07, 0xc3, 0x00, 0x00, 0xef, 0x00, 0x78, 0x00});
    static constexpr auto sid_83_02 =
        std::to_array<uint8_t>({0x80, 0xf1, 0x12, 0x07, 0xc3, 0x02, 0x00, 0x28, 0x00, 0x14, 0x00});
    static constexpr auto sid_83_03 = std::to_array<uint8_t>({0x82, 0xf1, 0x12, 0xc3, 0x03});

    static constexpr auto sid_27_01 = std::to_array<uint8_t>({0x80, 0xf1, 0x12, 0x05, 0x67, 0x01, 0xb3, 0x59, 0x2c});
    static constexpr auto sid_27_02 = std::to_array<uint8_t>({0x82, 0xf1, 0x12, 0x03, 0x67, 0x01, 0x34});
    static constexpr auto sid_27_10 =
        std::to_array<uint8_t>({0x80, 0xf1, 0x12, 0x06, 0x67, 0x10, 0x10, 0x10, 0x10, 0x10});
    static constexpr auto sid_27_11 = std::to_array<uint8_t>({0x82, 0xf1, 0x12, 0x03, 0x67, 0x11, 0x34});

    static constexpr auto sid_1a_90 =
        std::to_array<uint8_t>({0x13, 0x5a, 0x90, 0x57, 0x44, 0x42, 0x32, 0x31, 0x31, 0x32,
                                0x32, 0x36, 0x31, 0x41, 0x32, 0x39, 0x32, 0x38, 0x36, 0x39});
    static constexpr auto sid_21_09 = std::to_array<uint8_t>(
    ...
    while (simulate_obd_on)
    {
        ...
        if (received != "")
        {
            ...
            {
                output.append(reinterpret_cast<const char *>(sid_4d_ff_b4.data()),
                              static_cast<qsizetype>(sid_4d_ff_b4.size()));
            }
            /*
