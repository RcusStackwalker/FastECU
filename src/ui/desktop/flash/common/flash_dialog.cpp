...
FlashDialog::FlashDialog(std::unique_ptr<FlashWorkflow> workflow,
                         FlashOperation operation, const QString& filename, QWidget *parent)
    : QDialog(parent), workflow_(std::move(workflow)),
      ui_(std::make_unique<Ui::EcuOperationsWindow>())
{
    ui_->setupUi(this);
...
void FlashDialog::advance()
{
    while (true)
    {
        ...
        result_.outcome = error.kind == ErrorKind::Cancelled
                              ? FlashWorkflowOutcome::Cancelled
                              : FlashWorkflowOutcome::Failed;
...
{
    worker_ = std::make_unique<FlashWorker>(std::move(attempt.plan),
                                            std::move(attempt.executor),
                                            std::move(attempt.transport),
    ...
    connect(worker_.get(), &FlashWorker::phaseProgressChanged, this,
            ...
            {
                if (ui_->progressbar)
                    ui_->progressbar->setFormat(tr("Phase %1/%2 - %3: %p%")
                                                    .arg(index)
                                                    .arg(count)
                                                    .arg(phase));
