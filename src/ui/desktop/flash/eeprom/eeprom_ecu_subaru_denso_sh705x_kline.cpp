...
EepromEcuSubaruDensoSH705xKline::~EepromEcuSubaruDensoSH705xKline() = default;
...
void EepromEcuSubaruDensoSH705xKline::runAttempt(fastecu::flash::EepromReadMode mode)
{
    currentMode_ = mode;
    ...
    {
        // InvalidConfig/Unsupported: a static configuration problem (bad
        // MCU/kernel/definition), not a per-attempt ECU response issue --
        // retrying at a different mode would fail identically, so this
        // stops the sequence immediately without starting a FlashWorker.
    ...
    connect(worker_.get(), &fastecu::flash::FlashWorker::finished, this,
            &EepromEcuSubaruDensoSH705xKline::onWorkerFinished);

    worker_->start();
}

void EepromEcuSubaruDensoSH705xKline::onWorkerFinished(fastecu::flash::FlashWorkerResult result)
{
    ...
    // time, never overlapping (this dialog's core orchestration contract).
    worker_.reset();
    ...
    if (!result.success)
    {
        ...
        {
            // User-initiated cancellation (dialog closed mid-attempt): no
            // failure dialog, matches the Global Constraint table exactly.
            close();
            return;
        }
        ...
        {
            // Last attempt exhausted: show exactly one family-specific
            // failure dialog and stop.
        ...
        // the next mode without any dialog -- matches the legacy operation
        // class's loop, which never prompted on a failed attempt, only on a
        // successful-but-discarded one (see this file's class-level comment
        // and task-17-report.md for the full reasoning).
    ...
    {
        // Only Save updates ecuCalDef_->FullRomData -- a discarded (Ignore)
        // or auto-advanced (failed) attempt must never touch it.
    ...
    if (currentMode_ == fastecu::flash::EepromReadMode::Mode4)
    {
        // No more modes to try; same unsuccessful/no-selection outcome as
        // an exhausted failure sequence, but no failure dialog is warranted
        // here -- the user actively chose to discard a valid read, they did
        // not hit an ECU error.
        close();
        return;
    }
...
fastecu::Result<fastecu::flash::FlashPlan> EepromEcuSubaruDensoSH705xKline::buildPlan(
    ...
    fastecu::flash::FlashPlan plan)
{
    return std::make_unique<fastecu::flash::FlashWorker>(
        std::move(plan), std::make_unique<fastecu::flash::DensoSh705xEepromKlineExecutor>(),
        std::make_unique<fastecu::flash::DesktopKlineFlashTransport>(serial_),
        std::make_unique<QtClock>());
}

int EepromEcuSubaruDensoSH705xKline::confirm(const QString& title, const QString& text,
...
void EepromEcuSubaruDensoSH705xKline::showFailureDialog(fastecu::ErrorKind kind,
                                                        const QString& detail)
{
    ...
    case ErrorKind::Cancelled:
        ...
        // calling showFailureDialog(). No dialog for a cooperative
        // cancellation, per the Global Constraint table.
        break;
    case ErrorKind::Internal:
        // Verbatim legacy text -- the only failure message the pre-rewrite
        // dialog ever showed, preserved exactly for this kind.
        QMessageBox::warning(this, tr("ECU Operation"),
...
void EepromEcuSubaruDensoSH705xKline::closeEvent(QCloseEvent *event)
{
    if (worker_)
    {
        worker_->requestStop();
