 ...
 ******************************************************/
int FlashEcuSubaruDensoMC68HC16Y5_02_BDMOperation::read_mem(uint32_t start_addr, uint32_t length)
{
    QElapsedTimer timer;
    QByteArray output;
    QByteArray received;
    ...
    while (willget)
    {
        ...
        qDebug() << "Sent:" << output << bytes::toHex(output);
        received.clear();
        ...
        {
            mapdata.append(received);
            qDebug() << "Received:" << bytes::toHex(received);
        }
        else
        {
            qDebug() << "ERROR IN DATA RECEIVE!";
            qDebug() << "Received:" << received << bytes::toHex(received);
            return STATUS_ERROR;
        }
...
int FlashEcuSubaruDensoMC68HC16Y5_02_BDMOperation::write_mem()
{
    QByteArray output;
    QByteArray received;
    QByteArray msg;
    ...
    if (received.length() < cmd_wr_response.length() || received != cmd_wr_response)
    {
        ...
        qDebug() << "ERROR! Received:" << received << bytes::toHex(received);
        return STATUS_ERROR;
    }
