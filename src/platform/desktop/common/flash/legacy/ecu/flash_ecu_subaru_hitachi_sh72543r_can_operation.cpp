#include "src/platform/desktop/common/flash/legacy/ecu/flash_ecu_subaru_hitachi_sh72543r_can_operation.h"
...
FlashEcuSubaruHitachiSH72543rCanOperation::FlashEcuSubaruHitachiSH72543rCanOperation(
 ...
 * @return success
 */
int FlashEcuSubaruHitachiSH72543rCanOperation::connect_bootloader()
{
    QByteArray output;
    QByteArray received;
    QByteArray seed;
 ...
 * @return success
 */
int FlashEcuSubaruHitachiSH72543rCanOperation::read_mem(uint32_t start_addr, uint32_t length)
{
    QElapsedTimer timer;
    QByteArray output;
    QByteArray received;
...
int FlashEcuSubaruHitachiSH72543rCanOperation::write_mem(bool test_write_arg)
{
    ...
    std::array<int, 16> block_modified{0, 1, 1}; // assume blocks after 0x8000 are modified
 ...
 * @return success
 */
int FlashEcuSubaruHitachiSH72543rCanOperation::erase_mem()
{
    QByteArray output;
    QByteArray received;
    QByteArray seed;
 ...
 */
QByteArray FlashEcuSubaruHitachiSH72543rCanOperation::generate_can_seed_key(const QByteArray& requested_seed)
{
    QByteArray key;

    static constexpr auto keytogenerateindex =
        std::to_array<uint16_t>({// 1
                                 //        0x78B1, 0x4625, 0x201C, 0x9EA5,
                                 //        0xAD6B, 0x35F4, 0xFD21, 0x5E71,
                                 //        0xB046, 0x7F4A, 0x4B75, 0x93F9,
                                 //        0x1895, 0x8961, 0x3ECC, 0x862B
                                 // Normal2
                                 0x794B, 0x3CAF, 0x3019, 0x8B57, 0x52A0, 0xA77C, 0x38C9, 0xB0B5, 0x6520, 0x3B66, 0xA09D,
                                 0x2877, 0x479F, 0xB685, 0x7568, 0x84D7});
...
QByteArray FlashEcuSubaruHitachiSH72543rCanOperation::encrypt_payload(const QByteArray& buf, uint32_t len)
{
    QByteArray encrypted;

    static constexpr auto keytogenerateindex = std::to_array<uint16_t>({//	0x5FB1, 0xA7CA, 0x42DA, 0xB740
...
QByteArray FlashEcuSubaruHitachiSH72543rCanOperation::decrypt_payload(const QByteArray& buf, uint32_t len)
{
    QByteArray decrypt;

    static constexpr auto keytogenerateindex = std::to_array<uint16_t>({//	0xB740, 0x42DA, 0xA7CA, 0x5FB1
