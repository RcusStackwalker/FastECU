...
bool FlashEcuSubaruDensoSH7058CanDieselOperation::execute()
{
 ...
 * @return success
 */
int FlashEcuSubaruDensoSH7058CanDieselOperation::connect_bootloader()
{
    QByteArray output;
    QByteArray received;
    QByteArray seed;
...
/*
 * Read memory from Subaru Denso CAN (iso15765) 32bit ECUs, nisprog kernel
 *
 * @return success
 */
int FlashEcuSubaruDensoSH7058CanDieselOperation::read_mem(uint32_t start_addr, uint32_t length)
{
    QElapsedTimer timer;
    QByteArray output;
    QByteArray received;
 ...
 * @return
 */
int FlashEcuSubaruDensoSH7058CanDieselOperation::get_changed_blocks(const uint8_t *src, int *modified)
{
    unsigned blockno;
    QByteArray msg;
 ...
 * @return
 */
int FlashEcuSubaruDensoSH7058CanDieselOperation::check_romcrc(const uint8_t *src, uint32_t start, uint32_t len,
                                                              int *modified)
{
    QByteArray output;
    QByteArray received;
    ...
    if (received.length() > 5)
    {
        ...
        {
            ecucrc32 = ((uint8_t)received.at(9) << 24) | ((uint8_t)received.at(10) << 16) |
                       ((uint8_t)received.at(11) << 8) | (uint8_t)received.at(12);
        }
        else
        {
...
int FlashEcuSubaruDensoSH7058CanDieselOperation::init_flash_write()
{
    ...
    if (received.length() > 9)
    {
        ...
        {
            flashmessagesize = (uint8_t)received.at(9) << 24 | (uint8_t)received.at(10) << 16 |
                               (uint8_t)received.at(11) << 8 | (uint8_t)received.at(12) << 0;
            msg.clear();
    ...
    if (received.length() > 9)
    {
        ...
        {
            flashblocksize = (uint8_t)received.at(9) << 24 | (uint8_t)received.at(10) << 16 |
                             (uint8_t)received.at(11) << 8 | (uint8_t)received.at(12) << 0;
            msg.clear();
 ...
 * @return success
 */
int FlashEcuSubaruDensoSH7058CanDieselOperation::flash_block(const uint8_t *src, uint32_t start, uint32_t len)
{
    QByteArray output;
    QByteArray received;
    QByteArray msg;
 ...
 * @return kernel id
 */
QByteArray FlashEcuSubaruDensoSH7058CanDieselOperation::request_kernel_id()
{
    QByteArray output;
    QByteArray received;
    QByteArray kernelid;
