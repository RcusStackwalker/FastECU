...
FlashEcuSubaruDensoSH7058CanOperation::FlashEcuSubaruDensoSH7058CanOperation(SerialPortActions *serial,
...
bool FlashEcuSubaruDensoSH7058CanOperation::execute()
{
    ...
    {
        test_write = true;
        emit LOG_I("Test write memory with flashmethod '" + flash_method + "' and kernel '" + ecuCalDef->Kernel + "'",
                   true, true);
    }
 ...
 * @return success
 */
int FlashEcuSubaruDensoSH7058CanOperation::connect_bootloader()
{
    QByteArray output;
    QByteArray received;
    QByteArray seed;
    ...
    if (received.length() > 8)
    {
        if ((uint8_t)received.at(4) != ((SUB_KERNEL_START_COMM >> 8) & 0xFF) ||
            ...
            (uint8_t)received.at(8) != (SUB_KERNEL_ID | 0x40))
        {
            emit LOG_E("Wrong response from ECU: " +
                           FileActions::parse_nrc_message(received.mid(8, received.length() - 1)),
                       true, true);
        }
        else
        {
    ...
    if (flash_method.endsWith("_ecutek_racerom_alt"))
    {
        ...
        emit LOG_D("Value at RAM loc 0x" + QString::number(seed_alter_addr, 16) + " is: 0x" +
                       QString::number(ram_value, 16),
                   true, true);

        ram_value = read_ram_location(xor_bytes_addr);
        xor_byte_1 = ((ram_value >> 8) & 0xff);
        xor_byte_2 = (ram_value & 0xff);
        emit LOG_D("Value at RAM loc 0x" + QString::number(xor_bytes_addr, 16) + " is: 0x" +
                       QString::number(ram_value, 16),
                   true, true);

        emit LOG_I("Seed alter is: 0x" + QString::number(seed_alter, 16), true, true);
        emit LOG_I("XOR values are: 0x" + QString::number(xor_byte_1, 16) + " and 0x" + QString::number(xor_byte_2, 16),
                   true, true);
    }
...
uint32_t FlashEcuSubaruDensoSH7058CanOperation::read_ram_location(uint32_t loc)
{
    QByteArray output;
    QByteArray received;
    QByteArray response;
    ...
    uint32_t result_arg = 0;
    result_arg = (response.at(5) << 24) & 0xFF000000;
    result_arg += (response.at(6) << 16) & 0x00FF0000;
    result_arg += (response.at(7) << 8) & 0x0000FF00;
    result_arg += response.at(8) & 0x000000FF;
 ...
 * @return success
 */
int FlashEcuSubaruDensoSH7058CanOperation::upload_kernel(const QString& kernel_arg, uint32_t kernel_start_addr)
{
    ...
    {
        chk_sum += ((pl_encr.at(i) << 24) & 0xFF000000) | ((pl_encr.at(i + 1) << 16) & 0xFF0000) |
                   ((pl_encr.at(i + 2) << 8) & 0xFF00) | ((pl_encr.at(i + 3)) & 0xFF);
    }
 ...
 * @return success
 */
int FlashEcuSubaruDensoSH7058CanOperation::read_mem(uint32_t start_addr, uint32_t length)
{
    QElapsedTimer timer;
    QByteArray output;
    QByteArray received;
    ...
    while (willget)
    {
        ...
        if (received.length() > 8)
        {
            if ((uint8_t)received.at(4) != ((SUB_KERNEL_START_COMM >> 8) & 0xFF) ||
                (uint8_t)received.at(5) != (SUB_KERNEL_START_COMM & 0xFF) ||
                (uint8_t)received.at(8) != (SUB_KERNEL_READ_AREA | 0x40))
            {
                emit LOG_E("Wrong response from ECU: " +
                               FileActions::parse_nrc_message(received.mid(8, received.length() - 1)),
                           true, true);
 ...
 * @return success
 */
int FlashEcuSubaruDensoSH7058CanOperation::write_mem(bool test_write_arg)
{
    QByteArray filedata;
 ...
 * @return
 */
int FlashEcuSubaruDensoSH7058CanOperation::get_changed_blocks(const uint8_t *src, int *modified)
{
    unsigned blockno;
    QByteArray msg;
 ...
 * @return
 */
int FlashEcuSubaruDensoSH7058CanOperation::check_romcrc(const uint8_t *src, uint32_t addr, uint32_t len, int *modified)
{
    QByteArray output;
    QByteArray received;
    QByteArray msg;
    ...
    if (received.length() > 5)
    {
        if ((uint8_t)received.at(4) != ((SUB_KERNEL_START_COMM >> 8) & 0xFF) ||
            (uint8_t)received.at(5) != (SUB_KERNEL_START_COMM & 0xFF) ||
            (uint8_t)received.at(8) != (SUB_KERNEL_CRC | 0x40))
        {
    ...
    ecucrc32 = ((uint8_t)received.at(9) << 24) | ((uint8_t)received.at(10) << 16) | ((uint8_t)received.at(11) << 8) |
               (uint8_t)received.at(12);

    imgcrc32 = fastecu::checksum::crc32(src, pagesize);
    msg.clear();
    msg.append(QString("ROM CRC: 0x%1 IMG CRC: 0x%2")
                   .arg(ecucrc32, 8, 16, QLatin1Char('0'))
                   .arg(imgcrc32, 8, 16, QLatin1Char('0'))
                   .toUtf8());
...
int FlashEcuSubaruDensoSH7058CanOperation::init_flash_write()
{
    QByteArray output;
    QByteArray received;
    QByteArray msg;
    ...
    if (received.length() > 9)
    {
        if ((uint8_t)received.at(4) != ((SUB_KERNEL_START_COMM >> 8) & 0xFF) ||
            (uint8_t)received.at(5) != (SUB_KERNEL_START_COMM & 0xFF) ||
            (uint8_t)received.at(8) != (SUB_KERNEL_GET_MAX_MSG_SIZE | 0x40))
        {
            emit LOG_E("", false, true);
            emit LOG_E("Wrong response from ECU: " +
                           FileActions::parse_nrc_message(received.mid(8, received.length() - 1)),
    ...
    flashmessagesize = (uint8_t)received.at(9) << 24 | (uint8_t)received.at(10) << 16 | (uint8_t)received.at(11) << 8 |
                       (uint8_t)received.at(12) << 0;
    msg.clear();
    ...
    if (received.length() > 9)
    {
        if ((uint8_t)received.at(4) != ((SUB_KERNEL_START_COMM >> 8) & 0xFF) ||
            (uint8_t)received.at(5) != (SUB_KERNEL_START_COMM & 0xFF) ||
            (uint8_t)received.at(8) != (SUB_KERNEL_GET_MAX_BLK_SIZE | 0x40))
        {
            emit LOG_E("", false, true);
            emit LOG_E("Wrong response from ECU: " +
                           FileActions::parse_nrc_message(received.mid(8, received.length() - 1)),
    ...
    flashblocksize = (uint8_t)received.at(9) << 24 | (uint8_t)received.at(10) << 16 | (uint8_t)received.at(11) << 8 |
                     (uint8_t)received.at(12) << 0;
    msg.clear();
    ...
    if (received.length() > 5)
    {
        if ((uint8_t)received.at(4) != ((SUB_KERNEL_START_COMM >> 8) & 0xFF) ||
            (uint8_t)received.at(5) != (SUB_KERNEL_START_COMM & 0xFF) ||
            (uint8_t)received.at(8) != (SUB_KERNEL_CMD | 0x40))
        {
            emit LOG_E("Wrong response from ECU: " +
                           FileActions::parse_nrc_message(received.mid(8, received.length() - 1)),
                       true, true);
 ...
 * @return success
 */
int FlashEcuSubaruDensoSH7058CanOperation::reflash_block(const uint8_t *newdata, const struct flashdev_t *fdt,
                                                         unsigned blockno, bool test_write_arg)
{
    ...
    if (received.length() > 7)
    {
        if ((uint8_t)received.at(4) != ((SUB_KERNEL_START_COMM >> 8) & 0xFF) ||
            (uint8_t)received.at(5) != (SUB_KERNEL_START_COMM & 0xFF) ||
            (uint8_t)received.at(8) != (SUB_KERNEL_PROG_VOLT | 0x40))
        {
            emit LOG_E("", false, true);
            emit LOG_E("Wrong response from ECU: " +
                           FileActions::parse_nrc_message(received.mid(8, received.length() - 1)),
    ...
    {
        emit LOG_E("Reflash error! Do not panic, do not reset the ECU immediately. The kernel is most likely still "
 ...
 * @return success
 */
int FlashEcuSubaruDensoSH7058CanOperation::flash_block(const uint8_t *src, uint32_t start, uint32_t len)
{
    QByteArray output;
    QByteArray received;
    QByteArray msg;
    ...
    msg = QString("Flash page erase addr: 0x%1 len: 0x%2")
              .arg(block_start, 8, 16, QLatin1Char('0'))
              .arg(block_len, 8, 16, QLatin1Char('0'))
              .toUtf8();
    emit LOG_I(msg, true, true);
    ...
    if (received.length() > 8)
    {
        if ((uint8_t)received.at(4) != ((SUB_KERNEL_START_COMM >> 8) & 0xFF) ||
            (uint8_t)received.at(5) != (SUB_KERNEL_START_COMM & 0xFF) ||
            (uint8_t)received.at(8) != (SUB_KERNEL_BLANK_PAGE | 0x40))
        {
            emit LOG_E("", false, true);
            emit LOG_E("Wrong response from ECU: " +
                           FileActions::parse_nrc_message(received.mid(8, received.length() - 1)),
    ...
    msg = QString("Start flash write addr: 0x%1 len: 0x%2")
              .arg(block_start, 8, 16, QLatin1Char('0'))
              .arg(block_len, 8, 16, QLatin1Char('0'))
              .toUtf8();
    emit LOG_I(msg, true, true);
    ...
    while (remain)
    {
        ...
        if (received.length() > 8)
        {
            if ((uint8_t)received.at(4) != ((SUB_KERNEL_START_COMM >> 8) & 0xFF) ||
                (uint8_t)received.at(5) != (SUB_KERNEL_START_COMM & 0xFF) ||
                (uint8_t)received.at(8) != (SUB_KERNEL_WRITE_FLASH_BUFFER | 0x40))
            {
                emit LOG_E("Wrong response from ECU: " +
                               FileActions::parse_nrc_message(received.mid(8, received.length() - 1)),
                           true, true);
 ...
 */
QByteArray FlashEcuSubaruDensoSH7058CanOperation::generate_seed_key(const QByteArray& requested_seed)
{
    QByteArray key;

    static constexpr auto keytogenerateindex_1 =
        std::to_array<uint16_t>({0x78B1, 0x4625, 0x201C, 0x9EA5, 0xAD6B, 0x35F4, 0xFD21, 0x5E71, 0xB046, 0x7F4A, 0x4B75,
                                 0x93F9, 0x1895, 0x8961, 0x3ECC, 0x862B});

    static constexpr auto keytogenerateindex_2 =
        std::to_array<uint16_t>({0x24B9, 0x9D91, 0xFF0C, 0xB8D5, 0x15BB, 0xF998, 0x8723, 0x9E05, 0x7092, 0xD683, 0xBA03,
                                 0x59E1, 0x6136, 0x9B9A, 0x9CFB, 0x9DDB});
...
unsigned long long FlashEcuSubaruDensoSH7058CanOperation::decrypt_racerom_seed(unsigned long long base,
                                                                               unsigned long long exponent,
                                                                               unsigned long long modulus)
{
    unsigned long long result_arg = 1;
    base = base % modulus;
    while (exponent > 0)
    {
        if (exponent & 1)
        {
            result_arg = (result_arg * 1LL * base) % modulus;
        }
...
QByteArray FlashEcuSubaruDensoSH7058CanOperation::generate_ecutek_racerom_can_seed_key(const QByteArray& requested_seed)
{
 ...
 * @return seed key (4 bytes)
 */
QByteArray FlashEcuSubaruDensoSH7058CanOperation::generate_ecutek_seed_key(const QByteArray& requested_seed)
{
    QByteArray seed_key;
    ...
    static constexpr auto indextransformation =
        std::to_array<uint8_t>({0x4, 0x2, 0x5, 0x1, 0x8, 0xC, 0xD, 0x8, 0xA, 0xD, 0x2, 0xB, 0xF, 0x4, 0x0, 0x3,
                                0xB, 0x4, 0x6, 0x0, 0xF, 0x2, 0xD, 0x9, 0x5, 0xC, 0x1, 0xA, 0x3, 0xD, 0xE, 0x8});
    ...
    if (flash_method.endsWith("_ecutek_racerom_alt"))
    {
        uint32_t et_rr_seed_arg = 0;
        et_rr_seed_arg = ((uint8_t)seed_key.at(0) << 24) & 0xFF000000;
        et_rr_seed_arg += ((uint8_t)seed_key.at(1) << 16) & 0x00FF0000;
        et_rr_seed_arg += ((uint8_t)seed_key.at(2) << 8) & 0x0000FF00;
        et_rr_seed_arg += (uint8_t)seed_key.at(3) & 0x000000FF;

        et_rr_seed_arg = ((seed_alter ^ et_rr_seed_arg) ^ xor_byte_1) * xor_multi;
        et_rr_seed_arg = (et_rr_seed_arg ^ xor_byte_2) * xor_multi;

        seed_key.clear();
        seed_key.append((uint8_t)(et_rr_seed_arg >> 24) & 0xff);
        seed_key.append((uint8_t)(et_rr_seed_arg >> 16) & 0xff);
        seed_key.append((uint8_t)(et_rr_seed_arg >> 8) & 0xff);
        seed_key.append((uint8_t)(et_rr_seed_arg & 0xff));
 ...
 ***********************************/
QByteArray FlashEcuSubaruDensoSH7058CanOperation::generate_cobb_seed_key(const QByteArray& requested_seed)
{
    QByteArray key;

    // 2017 VA model
    static constexpr auto keytogenerateindex_1 =
        std::to_array<uint16_t>({0x9DDB, 0x9CFB, 0x9B9A, 0x6136, 0x59E1, 0xBA03, 0xD683, 0x7092, 0x9E05, 0x8723, 0xF998,
                                 0x15BB, 0xB8D5, 0xFF0C, 0x9D91, 0x24B9});

    // 2012 STi
    static constexpr auto keytogenerateindex_2 =
        std::to_array<uint16_t>({0x77C1, 0x80BB, 0xD5C1, 0x7A94, 0x11F0, 0x25FF, 0xC365, 0x10B4, 0x48DA, 0x6720, 0x3255,
                                 0xFA17, 0x60BF, 0x780E, 0x9C1D, 0x5A28});
 ...
 */
QByteArray FlashEcuSubaruDensoSH7058CanOperation::encrypt_payload(const QByteArray& buf, uint32_t len)
{
    QByteArray encrypted;

    static constexpr auto keytogenerateindex = std::to_array<uint16_t>({0xC85B, 0x32C0, 0xE282, 0x92A0});
...
QByteArray FlashEcuSubaruDensoSH7058CanOperation::decrypt_payload(const QByteArray& buf, uint32_t len)
{
    QByteArray decrypt;

    static constexpr auto keytogenerateindex = std::to_array<uint16_t>({0x92A0, 0xE282, 0x32C0, 0xC85B});
 ...
 * @return kernel id
 */
QByteArray FlashEcuSubaruDensoSH7058CanOperation::request_kernel_id()
{
    QByteArray output;
    QByteArray received;
    QByteArray kernelid;
