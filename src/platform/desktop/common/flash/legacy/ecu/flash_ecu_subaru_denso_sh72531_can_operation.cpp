#include "src/platform/desktop/common/flash/legacy/ecu/flash_ecu_subaru_denso_sh72531_can_operation.h"
...
FlashEcuSubaruDensoSH72531CanOperation::FlashEcuSubaruDensoSH72531CanOperation(
...
bool FlashEcuSubaruDensoSH72531CanOperation::execute()
{
    ...
    if (result_arg == STATUS_SUCCESS)
    {
        if (cmd_type == "read")
        {
            ...
            result_arg =
                read_memory(flashdevices[mcu_type_index].fblocks[0].start, flashdevices[mcu_type_index].romsize);
        }
 ...
 * @return success
 */
int FlashEcuSubaruDensoSH72531CanOperation::connect_bootloader()
{
    QByteArray output;
    QByteArray received;
    QByteArray seed;
    ...
    if (received.length() > 5)
    {
        if ((uint8_t)received.at(4) == 0xEA)
        {
            ...
            response_local.remove(0, 8);
            response_local.remove(5, response_local.length() - 5);
 ...
 * @return success
 */
int FlashEcuSubaruDensoSH72531CanOperation::read_memory(uint32_t start_addr, uint32_t length)
{
    QElapsedTimer timer;
    QByteArray output;
    QByteArray received;
    ...
    if (received.length() > 7)
    {
        if ((uint8_t)received.at(4) != 0x75 || (uint8_t)received.at(5) != 0x20 || (uint8_t)received.at(6) != 0x01 ||
            (uint8_t)received.at(7) != 0x01)
        {
            emit LOG_E("Wrong response from ECU: " +
                           FileActions::parse_nrc_message(received.mid(4, received.length() - 1)),
                       true, true);
...
int FlashEcuSubaruDensoSH72531CanOperation::write_memory(bool test_write_arg)
{
    QByteArray filedata;
 ...
 * @return success
 */
int FlashEcuSubaruDensoSH72531CanOperation::reflash_block(const uint8_t *newdata, const struct flashdev_t *fdt,
                                                          unsigned blockno, bool test_write_arg)
{
    ...
    if (received.length() != 7)
    {
        emit LOG_I("Wrong response from ECU: " + FileActions::parse_nrc_message(received.mid(4, received.length() - 1)),
                   true, true);
    ...
    else
    {
        ...
        {
            emit LOG_I("Wrong response from ECU: " +
                           FileActions::parse_nrc_message(received.mid(4, received.length() - 1)),
                       true, true);
            emit LOG_I("Checksum not verified", true, true);
            return STATUS_ERROR;
        ...
        else
        {
            ...
            {
                if ((uint8_t)received.at(4) != 0x71 || (uint8_t)received.at(5) != 0x01 ||
 ...
 * @return success
 */
int FlashEcuSubaruDensoSH72531CanOperation::erase_memory()
{
    QByteArray output;
    QByteArray received;
 ...
 */
QByteArray FlashEcuSubaruDensoSH72531CanOperation::generate_can_seed_key(const QByteArray& requested_seed)
{
    QByteArray key;

    const uint16_t keytogenerateindex[] = {0x78B1, 0x4625, 0x201C, 0x9EA5, 0xAD6B, 0x35F4, 0xFD21, 0x5E71,
...
QByteArray FlashEcuSubaruDensoSH72531CanOperation::encrypt_payload(const QByteArray& buf, uint32_t len)
{
...
QByteArray FlashEcuSubaruDensoSH72531CanOperation::decrypt_payload(const QByteArray& buf, uint32_t len)
{
    QByteArray decrypt;
