...
bool FlashEcuSubaruDensoSH705xKlineOperation::execute()
{
    int result_arg = STATUS_ERROR;
    emit progressChanged(0);

    // result_arg = init_flash_denso_kline_04(ecuCalDef, cmd_type);
 ...
 * @return success
 */
int FlashEcuSubaruDensoSH705xKlineOperation::connect_bootloader()
{
    QByteArray output;
    QByteArray received;
    QByteArray seed;
    ...
    {
        if ((uint8_t)received.at(0) != ((SUB_KERNEL_START_COMM >> 8) & 0xFF) ||
            (uint8_t)received.at(1) != (SUB_KERNEL_START_COMM & 0xFF) ||
            (uint8_t)received.at(4) != (SUB_KERNEL_ID | 0x40))
        {
 ...
 * @return success
 */
int FlashEcuSubaruDensoSH705xKlineOperation::upload_kernel(const QString& kernel_arg, uint32_t addr)
{
    ...
    {
        chk_sum += (((uint8_t)pl_encr.at(i) << 24) & 0xFF000000) | (((uint8_t)pl_encr.at(i + 1) << 16) & 0xFF0000) |
                   (((uint8_t)pl_encr.at(i + 2) << 8) & 0xFF00) | (((uint8_t)pl_encr.at(i + 3)) & 0xFF);
    }
 ...
 * @return success
 */
int FlashEcuSubaruDensoSH705xKlineOperation::read_mem(uint32_t start_addr, uint32_t length)
{
    QElapsedTimer timer;
    QByteArray output;
    QByteArray received;
    ...
    while (willget)
    {
        ...
        chksum = bytes::sum8(bytes::view(output));
        ...
        if (received.length() > 5)
        {
            if ((uint8_t)received.at(0) == ((SUB_KERNEL_START_COMM >> 8) & 0xFF) &&
                (uint8_t)received.at(1) == (SUB_KERNEL_START_COMM & 0xFF) &&
                (uint8_t)received.at(4) == (SUB_KERNEL_READ_AREA | 0x40))
            {
                received.remove(0, 5);
                received.remove(received.length() - 1, 1);
 ...
 * @return success
 */
int FlashEcuSubaruDensoSH705xKlineOperation::write_mem(bool test_write_arg)
{
    QByteArray filedata;
 ...
 * @return
 */
int FlashEcuSubaruDensoSH705xKlineOperation::get_changed_blocks(const uint8_t *src, int *modified)
{
    unsigned blockno;
    QByteArray msg;
 ...
 * @return
 */
int FlashEcuSubaruDensoSH705xKlineOperation::check_romcrc(const uint8_t *src, uint32_t addr, uint32_t len,
                                                          int *modified)
{
    QByteArray output;
    QByteArray received;
    ...
    if (received.length() > 5)
    {
        if ((uint8_t)received.at(0) == ((SUB_KERNEL_START_COMM >> 8) & 0xFF) &&
            (uint8_t)received.at(1) == (SUB_KERNEL_START_COMM & 0xFF) &&
            (uint8_t)received.at(4) == (SUB_KERNEL_CRC | 0x40))
        {
            ecucrc32 = ((uint8_t)received.at(5) << 24) | ((uint8_t)received.at(6) << 16) |
                       ((uint8_t)received.at(7) << 8) | (uint8_t)received.at(8);
        }
        else
        {
...
int FlashEcuSubaruDensoSH705xKlineOperation::init_flash_write()
{
    QByteArray output;
    QByteArray received;
    QByteArray msg;
    ...
    if (received.length() > 9)
    {
        if ((uint8_t)received.at(0) == ((SUB_KERNEL_START_COMM >> 8) & 0xFF) &&
            (uint8_t)received.at(1) == (SUB_KERNEL_START_COMM & 0xFF) &&
            (uint8_t)received.at(4) == (SUB_KERNEL_GET_MAX_MSG_SIZE | 0x40))
        {
            flashmessagesize = (uint8_t)received.at(5) << 24 | (uint8_t)received.at(6) << 16 |
                               (uint8_t)received.at(7) << 8 | (uint8_t)received.at(8) << 0;
            msg.clear();
    ...
    if (received.length() > 9)
    {
        if ((uint8_t)received.at(0) == ((SUB_KERNEL_START_COMM >> 8) & 0xFF) &&
            (uint8_t)received.at(1) == (SUB_KERNEL_START_COMM & 0xFF) &&
            (uint8_t)received.at(4) == (SUB_KERNEL_GET_MAX_BLK_SIZE | 0x40))
        {
            flashblocksize = (uint8_t)received.at(5) << 24 | (uint8_t)received.at(6) << 16 |
                             (uint8_t)received.at(7) << 8 | (uint8_t)received.at(8) << 0;
            msg.clear();
    ...
    if (received.length() > 5)
    {
        if ((uint8_t)received.at(0) == ((SUB_KERNEL_START_COMM >> 8) & 0xFF) &&
            (uint8_t)received.at(1) == (SUB_KERNEL_START_COMM & 0xFF) &&
            (uint8_t)received.at(4) == (SUB_KERNEL_CMD | 0x40))
        {
 ...
 *  @return
 */
int FlashEcuSubaruDensoSH705xKlineOperation::reflash_block(const uint8_t *newdata, const struct flashdev_t *fdt,
                                                           unsigned blockno, bool test_write_arg)
{
    ...
    if (received.length() > 9)
    {
        if ((uint8_t)received.at(0) == ((SUB_KERNEL_START_COMM >> 8) & 0xFF) &&
            (uint8_t)received.at(1) == (SUB_KERNEL_START_COMM & 0xFF) &&
            (uint8_t)received.at(4) == (SUB_KERNEL_PROG_VOLT | 0x40))
        {
 ...
 * @return
 */
int FlashEcuSubaruDensoSH705xKlineOperation::flash_block(const uint8_t *src, uint32_t start, uint32_t len)
{
    QByteArray output;
    QByteArray received;
    QByteArray msg;
    ...
    if (received.length() > 4)
    {
        if ((uint8_t)received.at(0) != ((SUB_KERNEL_START_COMM >> 8) & 0xFF) ||
            (uint8_t)received.at(1) != (SUB_KERNEL_START_COMM & 0xFF) ||
            (uint8_t)received.at(4) != (SUB_KERNEL_BLANK_PAGE | 0x40))
        {
            emit LOG_E("", false, true);
            emit LOG_E("Wrong response from ECU: " +
                           FileActions::parse_nrc_message(received.mid(8, received.length() - 1)),
    ...
    while (remain)
    {
        ...
        if (received.length() > 5)
        {
            if ((uint8_t)received.at(0) == ((SUB_KERNEL_START_COMM >> 8) & 0xFF) &&
                (uint8_t)received.at(1) == (SUB_KERNEL_START_COMM & 0xFF) &&
                (uint8_t)received.at(4) == (SUB_KERNEL_WRITE_FLASH_BUFFER | 0x40))
            {
 ...
 */
QByteArray FlashEcuSubaruDensoSH705xKlineOperation::send_sid_bf_ssm_init()
{
    QByteArray output;
    QByteArray received;
 ...
 */
QByteArray FlashEcuSubaruDensoSH705xKlineOperation::send_sid_81_start_communication()
{
    QByteArray output;
    QByteArray received;
 ...
 * @return received response
 */
QByteArray FlashEcuSubaruDensoSH705xKlineOperation::send_sid_83_request_timings()
{
    QByteArray output;
    QByteArray received;
 ...
 * @return seed (4 bytes)
 */
QByteArray FlashEcuSubaruDensoSH705xKlineOperation::send_sid_27_request_seed()
{
    QByteArray output;
    QByteArray received;
 ...
 * @return received response
 */
QByteArray FlashEcuSubaruDensoSH705xKlineOperation::send_sid_10_start_diagnostic()
{
    QByteArray output;
    QByteArray received;
...
QByteArray FlashEcuSubaruDensoSH705xKlineOperation::send_sid_34_request_upload(uint32_t addr, uint32_t len)
{
    QByteArray output;
    QByteArray received;
 ...
 * @return received response
 */
QByteArray FlashEcuSubaruDensoSH705xKlineOperation::send_sid_36_transferdata(uint32_t addr, const QByteArray& buf,
                                                                             uint32_t len)
{
    QByteArray output;
    QByteArray received;
...
QByteArray FlashEcuSubaruDensoSH705xKlineOperation::generate_seed_key(const QByteArray& requested_seed)
{
    ...
    static constexpr auto keytogenerateindex_1 =
        std::to_array<uint16_t>({0x53DA, 0x33BC, 0x72EB, 0x437D, 0x7CA3, 0x3382, 0x834F, 0x3608, 0xAFB8, 0x503D, 0xDBA3,
 ...
 * @return kernel id
 */
QByteArray FlashEcuSubaruDensoSH705xKlineOperation::request_kernel_id()
{
    QByteArray output;
    QByteArray received;
    QByteArray kernelid;
