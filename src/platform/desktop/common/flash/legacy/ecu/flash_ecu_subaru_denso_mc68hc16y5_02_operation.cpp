#include "src/platform/desktop/common/flash/legacy/ecu/flash_ecu_subaru_denso_mc68hc16y5_02_operation.h"
...
FlashEcuSubaruDensoMC68HC16Y5_02Operation::FlashEcuSubaruDensoMC68HC16Y5_02Operation(
...
bool FlashEcuSubaruDensoMC68HC16Y5_02Operation::execute()
{
    ...
    emit LOG_I("Connecting to Subaru 01-05 16-bit K-Line bootloader, please wait...", true, true);
    result = connect_bootloader();

    if (result == STATUS_SUCCESS && !kernel_alive)
    {
        emit externalLoggerMessage("Preparing, please wait...");
        emit LOG_I("Initializing Subaru 01-05 16-bit K-Line kernel upload, please wait...", true, true);
    ...
    if (result == STATUS_SUCCESS)
    {
        if (cmd_type == "read")
        {
            ...
            emit LOG_I("Reading ROM from Subaru 01-05 16-bit using K-Line", true, true);
        ...
        else if (cmd_type == "test_write" || cmd_type == "write")
        {
            ...
            emit LOG_I("Writing ROM to Subaru 01-05 16-bit using K-Line", true, true);
...
int FlashEcuSubaruDensoMC68HC16Y5_02Operation::connect_bootloader()
{
    QByteArray output;
    QByteArray received;
    ...
    {
        bootloader_init_response_wrx02_ok = bootloader_init_response_ecutek_wrx02_ok;
    }
    else if (flash_method.endsWith("_cobb"))
    {
        bootloader_init_response_wrx02_ok = bootloader_init_response_cobb_wrx02_ok;
    }
    else
    {
        bootloader_init_response_wrx02_ok = bootloader_init_response_stock_wrx02_ok;
    }

    if (received.length() < 3 || !check_received_message(bootloader_init_response_wrx02_ok, received))
    {
        emit LOG_E("Bad response from bootloader: " + bytes::toHex(received), true, true);
    }
    else
    {
...
int FlashEcuSubaruDensoMC68HC16Y5_02Operation::upload_kernel(const QString& kernel, uint32_t kernel_start_addr)
{
    QFile file(kernel);
    ...
    chk_sum = fastecu::checksum::checksum8(output, true);
 ...
 ******************************************************/
int FlashEcuSubaruDensoMC68HC16Y5_02Operation::read_mem(uint32_t start_addr, uint32_t length)
{
    QElapsedTimer timer;
    QByteArray output;
    QByteArray received;
 ...
 ******************************************************/
int FlashEcuSubaruDensoMC68HC16Y5_02Operation::write_mem(bool test_write)
{
    QByteArray filedata;
    QByteArray output;
    QByteArray received;
    ...
    if (filedata.length() < static_cast<qsizetype>(190 * 1024))
    {
 ...
 ******************************************************/
int FlashEcuSubaruDensoMC68HC16Y5_02Operation::get_changed_blocks(const uint8_t *src, int *modified)
{
    unsigned blockno;
    QByteArray msg;
    for (blockno = 0; blockno < flashdevices[mcu_type_index].numblocks; blockno++)
 ...
 ******************************************************/
int FlashEcuSubaruDensoMC68HC16Y5_02Operation::check_romcrc(const uint8_t *src, uint32_t start, uint32_t len, int *modified)
{
    QByteArray output;
    QByteArray received;
    QByteArray msg;
 ...
 *  @return
 */
int FlashEcuSubaruDensoMC68HC16Y5_02Operation::reflash_block(const uint8_t *newdata, const struct flashdev_t *fdt, unsigned blockno, bool test_write)
{
    int errval;
 ...
 * @return
 */
int FlashEcuSubaruDensoMC68HC16Y5_02Operation::flash_block(const uint8_t *src, uint32_t start, uint32_t len)
{
    uint32_t remain = len;
    ...
    if (!test_write)
    {
        ...
        // emit LOG_E("Wrong response from ECU: " + bytes::toHex(received), true, true);
        // return STATUS_ERROR;
    }
 ...
 *  Request kernel id
 ******************************************************/
QByteArray FlashEcuSubaruDensoMC68HC16Y5_02Operation::request_kernel_id()
{
    QByteArray output;
    QByteArray received;
    QByteArray msg;
 ...
 */
int FlashEcuSubaruDensoMC68HC16Y5_02Operation::check_received_message(const QByteArray& msg, const QByteArray& received)
{
