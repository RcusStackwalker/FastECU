#include "src/platform/desktop/common/flash/legacy/ecu/flash_ecu_subaru_denso_1n83m_4m_can_operation.h"
...
bool FlashEcuSubaruDenso1N83M_4MCanOperation::execute()
{
 ...
 * @return success
 */
int FlashEcuSubaruDenso1N83M_4MCanOperation::connect_bootloader()
{
    QByteArray output;
    QByteArray received;
    QByteArray seed;
 ...
 * @return success
 */
int FlashEcuSubaruDenso1N83M_4MCanOperation::read_memory(uint32_t start_addr, uint32_t length)
{
    QElapsedTimer timer;
    QByteArray output;
    QByteArray received;
...
int FlashEcuSubaruDenso1N83M_4MCanOperation::write_memory(bool test_write_arg)
{
    QByteArray filedata;
    ...
    if (bcnt)
    {
             ...
             blockno++) // hack so that only 1 flash loop done for the entire ROM above 0x8000
        {
            if (block_modified[blockno])
            {
                if (reflash_block(&data_array[0], &flashdevices[mcu_type_index], blockno, test_write_arg))
                {
 ...
 * @return success
 */
int FlashEcuSubaruDenso1N83M_4MCanOperation::reflash_block(const uint8_t *newdata, const struct flashdev_t *fdt,
 ...
 * @return success
 */
int FlashEcuSubaruDenso1N83M_4MCanOperation::erase_memory()
{
    QByteArray output;
    QByteArray received;
 ...
 */
QByteArray FlashEcuSubaruDenso1N83M_4MCanOperation::generate_can_seed_key(const QByteArray& requested_seed)
{
    QByteArray key;
...
QByteArray FlashEcuSubaruDenso1N83M_4MCanOperation::encrypt_payload(const QByteArray& buf, uint32_t len)
{
...
QByteArray FlashEcuSubaruDenso1N83M_4MCanOperation::decrypt_payload(const QByteArray& buf, uint32_t len)
{
    QByteArray decrypt;
