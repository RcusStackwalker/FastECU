...
QByteArray FlashTcuSubaruHitachiM32rKlineOperation::generate_kline_seed_key(const QByteArray& requested_seed)
{
    ...
    static constexpr auto keytogenerateindex =
        std::to_array<uint16_t>({0x0FE9, 0xCA58, 0x5E90, 0xDFF1, 0x690B, 0xF591, 0x1794, 0x5C7B, 0xA7BF, 0x98E5, 0x0B63,
                                 0xA1C9, 0x79BF, 0xF413, 0x82B1, 0xA895});
...
QByteArray FlashTcuSubaruHitachiM32rKlineOperation::send_sid_b0_block_write(uint32_t dataaddr, uint32_t datalen)
{
    ...
    emit LOG_I("Sent: " + bytes::toHex(SsmProtocol::addHeader(output, tester_id, target_id)), true, true);
    delay(100);
    received = serial->read_serial_data(receive_timeout);
    // received = QByteArray::fromHex("80f018f8e0");
    delay(100);
    emit LOG_I("Recd: " + bytes::toHex(received), true, true);
...
QByteArray FlashTcuSubaruHitachiM32rKlineOperation::send_sid_a0_block_read(uint32_t dataaddr, uint32_t datalen)
{
    ...
    received = serial->read_serial_data(receive_timeout);
    // received =
    // QByteArray::fromHex("80f01861e06000f000a04115ec6000f000a04115f06000f000a04115f46000f000a04115f86000f000a04115fc1fcef0002e7ff000fe00087e6000f000fe00022684ad806060072004fe0003e07e02f0002eef1fce6200f00081c200ff600401407d0e1081ab");
    delay(100);
    emit LOG_I("Recd: " + bytes::toHex(received), true, true);
