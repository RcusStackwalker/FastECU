...
int FlashEcuSubaruHitachiM32rJtagOperation::read_mem(uint32_t start_addr, uint32_t length)
{
...
void FlashEcuSubaruHitachiM32rJtagOperation::hard_reset_jtag()
{
    QByteArray output;
    QByteArray received;
...
int FlashEcuSubaruHitachiM32rJtagOperation::read_idcode()
{
    ...
    if (received.length() > 4)
    {
        if ((uint8_t)received.at(0) != ((SUB_KERNEL_START_COMM >> 8) & 0xFF) ||
            ...
            (uint8_t)received.at(4) != (SUB_KERNEL_ID + 0x40) || (uint8_t)received.at(8) != SUB_KERNEL_JTAG_IR_ACK)
        {
...
int FlashEcuSubaruHitachiM32rJtagOperation::read_usercode()
{
    QByteArray output;
    QByteArray received;
    ...
    if (received.length() > 4)
    {
        if ((uint8_t)received.at(0) != ((SUB_KERNEL_START_COMM >> 8) & 0xFF) ||
            (uint8_t)received.at(1) != (SUB_KERNEL_START_COMM & 0xFF) ||
            (uint8_t)received.at(4) != (SUB_KERNEL_READ_USERCODE + 0x40) ||
            (uint8_t)received.at(8) != SUB_KERNEL_JTAG_IR_ACK)
        {
            emit LOG_E("Wrong response from ECU: " +
                           FileActions::parse_nrc_message(received.mid(4, received.length() - 1)),
                       true, true);
...
int FlashEcuSubaruHitachiM32rJtagOperation::read_tool_rom_code()
{
    QByteArray output;
    QByteArray received;
    ...
    /*
        ...
        if (received.length() > 4)
        {
            if ((uint8_t)received.at(0) != ((SUB_KERNEL_START_COMM >> 8) & 0xFF) || (uint8_t)received.at(1) !=
       (SUB_KERNEL_START_COMM & 0xFF) || (uint8_t)received.at(4) != (SUB_KERNEL_JTAG_COMMAND + 0x40) ||
       (uint8_t)received.at(8) != SUB_KERNEL_JTAG_IR_ACK)
            {
                emit LOG_E("Wrong response from ECU: " + FileActions::parse_nrc_message(received.mid(4,
       received.length()-1)), true, true);
    ...
    */
    ...
    while (received.length())
    {
        if (received.length() > 4)
        {
            if ((uint8_t)received.at(0) != ((SUB_KERNEL_START_COMM >> 8) & 0xFF) ||
                (uint8_t)received.at(1) != (SUB_KERNEL_START_COMM & 0xFF) ||
                (uint8_t)received.at(4) != (SUB_KERNEL_JTAG_COMMAND + 0x40) ||
                (uint8_t)received.at(8) != SUB_KERNEL_JTAG_IR_ACK)
            {
                emit LOG_E("Wrong response from ECU: " +
                               FileActions::parse_nrc_message(received.mid(4, received.length() - 1)),
...
void FlashEcuSubaruHitachiM32rJtagOperation::set_rtdenb()
{
...
QByteArray FlashEcuSubaruHitachiM32rJtagOperation::read_response()
{
    QByteArray received;
    QByteArray response;
 ...
 */
QByteArray FlashEcuSubaruHitachiM32rJtagOperation::add_header(QByteArray output)
{
    QByteArray received;
    QByteArray msg;
