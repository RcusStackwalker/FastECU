...
{
    return runOnBackend([this]
                        { return m_backend->get_serial_port_parity(); });
}
bool SerialPortActions::set_serial_port_parity(uint8_t parity)
{
    return runOnBackend([this, parity]
                        { return m_backend->set_serial_port_parity(parity); });
}
...
bool SerialPortActions::set_serial_port_baudrate(const QString& value)
{
    ...
    return runOnBackend([this, value]
                        { return m_backend->set_serial_port_baudrate(value); });
}
...
{
    return runOnBackend([this, parameter, value]
                        { return m_backend->set_kline_timings(parameter, value); });
}
...
int SerialPortActions::fast_init(const QByteArray& output)
{
    set_comm_busy(true);
    return runOnBackend([this, output]
                        { return m_backend->fast_init(output); });
}
...
QByteArray SerialPortActions::write_serial_data_echo_check(const QByteArray& output)
{
    ...
    emit LOG_D("Sent: " + parse_message_to_hex(output.mid(0, 20)), true, true);
    return runOnBackend([this, output]
                        { return m_backend->write_serial_data_echo_check(output); });
}

bool SerialPortActions::get_is_tx_done()
{
    return runOnBackend([this]
                        { return m_backend->get_is_tx_done(); });
}
...
{
    return runOnBackend([this]
                        { return m_backend->check_serial_ports(); });
}

QString SerialPortActions::open_serial_port()
{
    return runOnBackend([this]
                        { return m_backend->open_serial_port(); });
}
