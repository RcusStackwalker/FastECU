...
constexpr std::chrono::milliseconds kLongTimeout{2000};   // serial_read_timeout, and read_memory's
...
// the re-read at line 1329. UdsClient substitutes pending_timeout for that
// second read, whose 3000 ms default is not this family's number, so the
// policy pins it to 500 as well.
constexpr uds::ExchangePolicy kChecksumPolicy{.read_timeout = kReceiveTimeout, .pending_timeout = kReceiveTimeout};
...
Status reflash_block(Ctx& ctx, bytes::ByteView image, const MemoryRegion& block, PhaseReporter& progress)
{
    ...
    for (int attempt = 0; attempt < 6; ++attempt)
    {
        if (Result<bytes::Bytes> closed =
            ...
            closed.has_value())
        {
            // Legacy appends the reply's hex to this line (line 1285);
