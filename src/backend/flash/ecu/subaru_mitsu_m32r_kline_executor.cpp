...
{
    static constexpr std::uint16_t kIndex[] = {0x8519, 0x5c53, 0xc0e9, 0x2452,
                                               0x1e68, 0x6feb, 0x2648, 0x81e2, 0x8ce4, 0x953b, 0x1ca9, 0x6180,
                                               0xb85e, 0x5109, 0xdb3c, 0x3cf2};
    static constexpr std::uint8_t kTransform[] = {0x5, 0x6, 0x7, 0x1, 0x9, 0xc, 0xd, 0x8,
...
{
    static constexpr std::uint16_t kIndex[] = {0x25b5, 0x3875, 0xca11, 0x2680};
    static constexpr std::uint8_t kTransform[] = {0x5, 0x6, 0x7, 0x1, 0x9, 0xc, 0xd, 0x8,
                                                  0xa, 0xd, 0x2, 0xb, 0xf, 0x4, 0x0, 0x3, 0xb, 0x4, 0x6, 0x0, 0xf, 0x2, 0xd, 0x9,
                                                  0x5, 0xc, 0x1, 0xa, 0x3, 0xd, 0xe, 0x8};
                              ...
                              const SubaruMitsuM32rKlinePlan& p)
{
    ...
    if (auto slept = clock.sleep(100, cancellation); !slept)
    {
    ...
    if (auto valid = expect_service(*id, {0xff}); !valid)
    {
    ...
    if (auto s = expect_service(*seed_response, {0x67, 0x01}); !s)
    {
        return std::unexpected(s.error());
    }
    ...
    if (auto s = request(std::move(key_request), {0x67, 0x02}); !s)
    {
        return std::unexpected(s.error());
    }
    events.log(LogLevel::Info, "Seed key ok");
    events.log(LogLevel::Info, "Set session mode");
    if (auto s = request({0x10, 0x85, 0x02}, {0x50}); !s)
    {
        return std::unexpected(s.error());
    }
                              ...
                              const SubaruMitsuM32rKlinePlan& p, MemoryRegion region)
{
    ...
    for (std::uint32_t offset = 0; offset < region.length; offset += p.chunk_size)
    {
        ...
        bytes::Bytes request{0xa0, 0x00, 0x20,
                             static_cast<bytes::Byte>(address >> 16), static_cast<bytes::Byte>(address >> 8),
                             static_cast<bytes::Byte>(address), static_cast<bytes::Byte>(p.chunk_size - 1)};
                 ...
                 const SubaruMitsuM32rKlinePlan& p, const FlashPlan& plan)
{
    ...
    if (auto baud = transport.setBaud(p.flash_baud); !baud)
    {
        return baud;
    }
    ...
    if (auto s = expect_service(*setup, {0x74}); !s)
    {
        return s;
    }
    ...
    if (auto s = expect_service(*erased, {0x71}); !s)
    {
        return s;
    }
    if (cancellation.cancelled())
    ...
    if (auto slept = clock.sleep(1000, cancellation); !slept)
    {
        return slept;
    }
    ...
    if (auto s = expect_service(*checksum, {0x71, 0x01, 0x02}); !s)
    {
        return s;
    }
    ...
    const ICancellationToken& cancellation, IEventSink& events)
{
    ...
    if (auto valid = validate_subaru_mitsu_m32r_kline_plan(plan); !valid)
    {
        return std::unexpected(valid.error());
    }
    if (cancellation.cancelled())
    ...
    Result<FlashExecutionResult> outcome = [&]() -> Result<FlashExecutionResult>
    {
        ...
        auto rom_id = handshake(*kline, clock, cancellation, events, p);
        if (!rom_id)
        {
        ...
        if (plan.operation() == FlashOperation::Read)
        {
            ...
            auto bytes = read_rom(*kline, cancellation, events, p, plan.transfer_region());
            if (!bytes)
            {
        ...
        if (auto status = write_rom(*kline, clock, cancellation, events, p, plan); !status)
        {
