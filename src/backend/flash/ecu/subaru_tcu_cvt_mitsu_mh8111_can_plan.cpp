...
Status validate_identity(std::string_view protocol, std::string_view mcu)
{
    if (protocol != kProtocol)
    {
        return fail(InvalidConfig,
                    std::format("Unsupported Subaru TCU CVT Mitsu MH8111 CAN protocol: {}",
                                protocol));
    }
...
Status validate_subaru_tcu_cvt_mitsu_mh8111_can_plan(const FlashPlan& plan)
{
    ...
    if (plan.family() != FlashFamily::SubaruTcuCvtMitsuMh8111Can ||
...
Result<FlashPlan> build_subaru_tcu_cvt_mitsu_mh8111_can_plan(FlashOperation operation,
                                                             ...
                                                             std::optional<bytes::Bytes> image)
{
    ...
    {
        return fail(InvalidConfig,
                    std::format("ROM file must be exactly 0x180000 bytes; got 0x{:x} bytes",
                                image->size()));
    }
