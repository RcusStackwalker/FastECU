...
FileActions::EcuCalDefStructure *FileActions::open_subaru_rom_file(FileActions::EcuCalDefStructure *ecuCalDef, QString filename)
{
    ...
    if (ecuCalDef->use_romraider_definition || ecuCalDef->use_ecuflash_definition)
    {
        ...
        auto catalog = build_definition_catalog(matchedFormat);
        if (catalog.has_value())
        {
            auto romDefinition =
                definitionService_.load(*catalog, matchedFormat, ecuCalDef->RomId.toStdString());
            if (romDefinition.has_value())
            {
                ...
                calibrationAdapter_.compute_map_cell_values(
                    *ecuCalDef, *romDefinition, ecuCalDef->RomInfo.at(FlashMethod),
                    float_precision);
            }
        }
    }
